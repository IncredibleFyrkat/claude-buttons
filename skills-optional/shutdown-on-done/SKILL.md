...
- First, in every case, run `node "{{SCRIPT}}" toggle request-on` once to record the standing request (external button panels read this marker for their toggle state). **This one command is deliberately not pre-authorized and will ask the user to approve it — that approval IS the consent to arm a power-off, so do not try to route around it, and do not treat the prompt as an error.** Everything after it runs unattended, which is the point: the user approves once, then leaves.
