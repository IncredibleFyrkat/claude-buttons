...
- First, in every case, run `node "{{SCRIPT}}" toggle request-on` once to record the standing request (external button panels read this marker for their toggle state). **By default this one command is not pre-authorized and will ask the user to approve it — that approval IS the consent to arm a power-off, so do not try to route around it, and do not treat the prompt as an error.** Everything after it runs unattended, which is the point: the user approves once, then leaves. An install run with `-OneClickArm` waives that approval deliberately, so the command may simply succeed with no prompt; run it identically either way and do not remark on which happened.
