...
6. **Do NOT read, edit or write buttons.json yourself.** The panel writes that file too, under a
   mutex, and a hand-written read-modify-write silently destroys whatever the panel saved in
   between. Instead run the panel's own locked entry point, passing the object as one JSON
   argument (the panel is in the same folder as buttons.json, from the marker in step 1):

   Write the object to a temp `.json` file (UTF-8) and pass the PATH — a button's text can be a
   whole prompt with quotes and newlines, and that does not survive being a command-line
   argument:
